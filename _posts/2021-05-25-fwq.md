...
记录一下项目部署练习以来的总结吧，包括前后端项目的部署，nginx的转发，ssl证书配置
...
##### 5、nodejs虽然已经安装成功了，但是还不能全局使用node命令，需要建立软连接
...
##### 1⃣️ source sql文件时候报错  does't exit
...
##### 4⃣️ silly-datetime模块的依赖没安装上导致npm start 报错

单独安装一遍依赖



##### 5⃣️ 413 Request Entity Too Large
